    ...
    <div class="lg:col-span-2">
        ...
        @if($latest->count() > 0)
            ...
            <div class="mb-6">
                ...
                <a href="{{ route('post.detail', $firstPost->id) }}" class="text-blue-600 text-sm no-underline">Xem chi tiết</a>
            </div>
        @endif
            ...
            @foreach($latest as $post)
                <div>
                    <a href="{{ route('post.detail', $post->id) }}">
                        <img src="{{ asset('image/' . $post->image) }}" alt="{{ $post->title }}" class="w-full h-40 object-cover mb-3 rounded">
                    </a>
                    <a href="{{ route('post.detail', $post->id) }}" class="block text-black no-underline hover:text-blue-600">
                        <h3 class="font-semibold text-xl mb-2">{{ $post->title }}</h3>
                    </a>
                    <p>{{ Str::limit($post->description, 100) }}</p>
                    <a href="{{ route('post.detail', $post->id) }}" class="text-blue-600 text-sm no-underline">Xem chi tiết</a>
                </div>
            @endforeach
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                @foreach($highlighted as $post)
                    <div>
                        ...
                        <a href="{{ route('post.detail', $post->id) }}" class="block text-black no-underline hover:text-blue-600">
                            <h4 class="font-semibold text-sm">{{ Str::limit($post->title, 50) }}</h4>
                        </a>
                    </div>
                @endforeach
            </div>
    ...
    <div>
        ...
        <ul class="space-y-3 text-sm">
            @foreach($popular as $post)
                <li>
                    <a href="{{ route('post.detail', $post->id) }}" class="text-black no-underline hover:text-blue-600">
                        {{ $post->title }}
                    </a>
                </li>
            @endforeach
